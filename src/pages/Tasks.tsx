...
import type { TaskBucket, TaskItem } from '@/types/db'
import { IconX } from '@/components/icons'

function DuePicker({ task, tz, onChange }: { task: TaskItem; tz: string; onChange: (d: string | null) => void }) {
  const [open, setOpen] = useState(false)
  const today = todayKey(tz)
  const overdue = !!task.due_date && task.due_date < today && task.status === 'open'
  const set = (d: string | null) => { setOpen(false); onChange(d) }
  return (
    <div className="relative">
      <button
        className={`rounded px-1.5 py-0.5 text-xs ${task.due_date ? (overdue ? 'bg-danger-soft text-danger' : 'bg-task-soft text-task') : 'text-faint hover:bg-hover'}`}
        aria-haspopup="menu" aria-expanded={open} onClick={() => setOpen((v) => !v)}
      >
        {task.due_date ? (overdue ? `Overdue · ${formatDueDate(task.due_date, tz)}` : formatDueDate(task.due_date, tz)) : 'Add date'}
      </button>
      {open && (
        <>
          <div className="fixed inset-0 z-20" onClick={() => setOpen(false)} />
          <div role="menu" className="pop absolute right-0 z-30 mt-1 w-44 glass-strong rounded-xl p-1">
            {([['Today', today], ['Tomorrow', addDays(today, 1)], ['Next week', addDays(today, 7)]] as const).map(([l, d]) => (
              <button key={l} role="menuitem" className="block w-full rounded px-3 py-1.5 text-left text-sm hover:bg-hover" onClick={() => set(d)}>{l}</button>
            ))}
            <label className="block px-3 py-1.5 text-sm">Pick a date
              <input type="date" defaultValue={task.due_date ?? ''} className="mt-1 w-full rounded border border-line bg-bg px-1.5 py-1 text-sm" onChange={(e) => e.target.value && set(e.target.value)} />
            </label>
            {task.due_date && <button role="menuitem" className="block w-full rounded px-3 py-1.5 text-left text-sm text-muted hover:bg-hover" onClick={() => set(null)}>No date</button>}
          </div>
        </>
      )}
    </div>
...
function TaskRow({ t, tz, onTick, onDue, onDelete }: { t: TaskItem; tz: string; onTick: (t: TaskItem, done: boolean) => void; onDue: (t: TaskItem, d: string | null) => void; onDelete: (t: TaskItem) => void }) {
  ...
  return (
    <li className="group flex items-start gap-3 rounded-lg px-2 py-2 hover:bg-hover">
      ...
      {!done && <DuePicker task={t} tz={tz} onChange={(d) => onDue(t, d)} />}
      {t.source === 'standalone' && (
...
function Section({ title, bucket, tz, defaultOpen = true, tone, onTick, onDue, onDelete }: {
  title: string; bucket: TaskBucket; tz: string; defaultOpen?: boolean; tone?: string
  onTick: (t: TaskItem, done: boolean) => void; onDue: (t: TaskItem, d: string | null) => void; onDelete: (t: TaskItem) => void
}) {
  ...
  return (
    <Card className="mb-4 rise" title={<span className={tone}>{title}{q.data ? <span className="ml-2 text-faint normal-case tracking-normal">{q.data.length}</span> : null}</span>}
      ...
      {open && (
        ...
        : <ul className="-mx-2">{q.data.map((t) => <TaskRow key={t.id} t={t} tz={tz} onTick={onTick} onDue={onDue} onDelete={onDelete} />)}</ul>
      )}
    </Card>
  )
}
...
export function Tasks() {
  ...
  const due = async (t: TaskItem, d: string | null) => { try { await updateTask(t.id, { due_date: d }); refresh() } catch { toast('Couldn’t set the date.', { kind: 'error' }) } }
  ...
  return (
    <div className="h-full overflow-y-auto">
      <div className="page-top mx-auto max-w-3xl px-4 pb-32 lg:px-6 lg:pb-8">
        ...
        <Section title="Today & overdue" bucket="today" tz={tz} onTick={tick} onDue={due} onDelete={del} />
        <Section title="Upcoming" bucket="upcoming" tz={tz} onTick={tick} onDue={due} onDelete={del} />
        <Section title="No date" bucket="no_date" tz={tz} onTick={tick} onDue={due} onDelete={del} />
        <Section title="Completed" bucket="completed" tz={tz} defaultOpen={false} onTick={tick} onDue={due} onDelete={del} />
      </div>
    </div>
  )
}
