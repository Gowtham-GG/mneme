...
import { createHabit, EVERY_DAY, listHabits, updateHabit, type HabitInput } from '@/api/habits'
...
function HabitRow({ h, onSave, onRemove }: { h: Habit; onSave: (p: Partial<HabitInput>) => void; onRemove: () => void }) {
    ...
    <div className="h-full overflow-y-auto">
      <div className="page-top mx-auto max-w-3xl px-4 pb-32 lg:px-6 lg:pb-8">
        ...
        <div className="glass rise rounded-2xl px-4">
          ...
          <ul>
            {q.data?.map((h) => (
              <HabitRow key={h.id} h={h} onSave={(p) => void run(() => updateHabit(h.id, p))} onRemove={() => remove(h)} />
            ))}
          </ul>
        </div>
      </div>
